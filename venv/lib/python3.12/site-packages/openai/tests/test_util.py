...
def api_key_file():
    ...
    try:
        with NamedTemporaryFile(prefix="openai-api-key", mode="wt") as tmp:
            # TODO: The 'openai.api_key_path' option isn't read in the client API. You will need to pass it when you instantiate the client, e.g. 'OpenAI(api_key_path=tmp.name)'
            # openai.api_key_path = tmp.name
            yield tmp
    finally:
        # TODO: The 'openai.api_key_path' option isn't read in the client API. You will need to pass it when you instantiate the client, e.g. 'OpenAI(api_key_path=saved_path)'

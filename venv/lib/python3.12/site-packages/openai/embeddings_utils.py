...
def get_embedding(text: str, engine="text-similarity-davinci-001") -> List[float]:
    ...
    return client.embeddings.create(input=[text], engine=engine)["data"][0]["embedding"]
...
async def aget_embedding(
    ...
    return (await aclient.embeddings.create(input=[text], engine=engine))["data"][0][
        "embedding"
    ]
...
def get_embeddings(
    ...
    data = client.embeddings.create(input=list_of_text, engine=engine).data
...
async def aget_embeddings(
    ...
    data = (await aclient.embeddings.create(input=list_of_text, engine=engine)).data
    data = sorted(data, key=lambda x: x["index"])  # maintain the same order as input.
    return [d["embedding"] for d in data]
